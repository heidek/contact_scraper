...
def get_zip_city(soup):
    ...
    try:
        ...
        for zip_explicit in zip_explicits:
            city, state = zip_to_city(zip_explicit[-5:])
            if city:
    ...
    except TimeoutError as e:  # Cookies failed to delete themselves
        pass
        print("something went awry")
...
def get_phone(soup):
    ...
    try:
        if not soup:
            return None
        phone = re.findall(r'\(?\d{3}\)?[\s.-]?\d{3}[\s.-]?\d{4}', soup.text)[0].strip()
    except TypeError as e:
        print(e)
    return phone


def zip_to_city(code):
    """
